...
const onTabControllerLinkClicked = (category) => {
  if (
    currentSection.value &&
    currentSection.value.category.id === category.id
  ) {
...
const _onSectionChanged = () => {
  ...
  if (isDesktopLayout.value) _resetScroll();
  else _resetScrollForMobile();
};
...
const _onScrollResetTriggered = () => {
  if (!shouldResetScroll.value) return;

  const currentSectionEl = document.getElementById(
    currentSection.value?.htmlId || ""
  );
  if (currentSectionEl) {
...
const _resetScrollForMobile = () => {
  ...
  const navMobileHeaderElHeight =
    navMobileHeaderEl?.getBoundingClientRect().height || 0;

  if (window.scrollY < navMobileHeaderElHeight * 0.9) return;

  const previousSectionHadPicker =
    previousSection.value?.category.sections.length >= 2;
  const currentSectionWillHavePicker =
    currentSection.value?.category.sections.length >= 2;

  const pickerHeight =
    window.innerWidth <= constants.BOOTSTRAP_BREAKPOINTS.sm
