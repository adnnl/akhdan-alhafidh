<template>
  <nav
    class="nav-mobile-tab-controller"
    :class="
      shouldAddChromeBottomOffset
        ? 'nav-mobile-tab-controller-with-bottom-offset'
        : ''
    ">
    <button
      v-if="shouldDisplayArrows && activeOptionIndex > 0"
    ...
    <button
      v-if="shouldDisplayArrows && activeOptionIndex < options.length - 2"
      class="nav-mobile-tab-button"
      @click="_onNextButton">
...
const visibleOptions = computed(() => {
  ...
  const activeOption =
    props.options.find((option) => option.active) || props.options[0];
