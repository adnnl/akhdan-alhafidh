...
const navigateToSection = (section) => {
  ...
  if (window.location.hash === "#" + section.urlHashId) return;
...
const _onHashChanged = () => {
  if (!didLoadAllJsonFiles.value) return;

  const hash = window.location.hash.replace("#", "");
  const targetSection = sections.value.find(
    (section) => section.urlHashId === hash
  );

  console.log("HASH:", hash);
  console.log(
    "SECTIONS:",
    sections.value.map((s) => s.urlHashId)
  );
...
const _onViewportChanged = (showSpinner) => {
  ...
  if (
    !showSpinner ||
    presentationMode.value === constants.PresentationModes.ONE_AT_ONCE
  )
    return;
